...
In MANUAL mode:
    - Arrow up or w key: accelerate
    - Arrow down or s key: brake
    - Arrow left or a key: steer left
    - Arrow right or d key: steer right

In AI mode:
 - Pressing any arrow key will take control of the car, and change to manual mode
 - A, D, W and S keys are used to provide indications to the model 
       - A: "Left"
       - D: "Right"
       - W: "Straight"
       - S: "LaneFollow"
...
In AUTO mode:
 - Pressing any of the following keys (Arrows, A, W, S or D) will take control of the car, and change to manual mode
"""
...
class ManualControlNode(Node):
    ...
    def __init__(self):
        ...
        self.pub_action = self.create_publisher(String, "action", qos_profile=self.qos_policy)
    ...
    def _procces_keys_manual(self, keys, milliseconds):
    ...
    def _parse_vehicle_keys(self):
        ...
        if self.mode == Mode.AUTO:
            # If any key is pressed, we change the mode to MANUAL
            if keys[K_UP] or keys[K_w] or keys[K_DOWN] or keys[K_s] or keys[K_LEFT] or keys[K_a] or keys[K_RIGHT] or keys[K_d]:
                self.mode = Mode.MANUAL
                self.get_logger().info("Mode set to MANUAL")

        if self.mode != Mode.AI:
            self._procces_keys_manual(keys, milliseconds)
        else:
            # In AI mode, we use the keys to indicate the action
            if keys[K_a]:
                self.tm_action = 'Left'
            elif keys[K_d]:
                self.tm_action = 'Right'
    ...
    def timer_callback(self):
        ...
        string_msg = String()
        string_msg.data = self.tm_action
        self.pub_action.publish(string_msg)
